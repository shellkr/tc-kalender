...
export function renderListView(session: any, startDateParam?: string) {
  ...
  filteredEvents.forEach((event: any) => {
    ...
    if (!currentWeek || currentWeek.weekNumber !== weekNumber) {
      currentWeek = { weekNumber, events: [] };
      weekGroups.push(currentWeek);
    }
    currentWeek.events.push(event);
  });
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              ` : weekGroups.map(weekGroup => {
                return weekGroup.events.map((event: any, idx: number) => {
                  const date = new Date(event.start);
                  ...
                  // Show week number only on first row of each week
                  const showWeekNumber = idx === 0;
                  const borderClass = idx === 0 ? 'border-t-2 border-gray-500' : 'border-t';
